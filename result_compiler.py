...
def get_percent_gt(col_data, dist):
    sum = 0
    for i in col_data:
        if i > dist:
            sum += 1
...
def summarize_groups(dir, group_files, missing):
    ...
    for file in group_files:
        ...
        for i, elem in enumerate(header):
            if '_symm' in elem and not 'nj' in elem:
                row = []
                matrix.append(row)
                ...
                perc_gt_2 = get_percent_gt(col_data, 2)
        ...
        try:
            ...
            #r("barcolors=sample(colors(), length(rownames))")
            r("barcolors=c('red', 'orchid', 'palegreen', 'paleturquoise', 'palevioletred', 'peachpuff', 'purple', 'royalblue', 'salmon', 'yellow', 'navy', 'magenta', 'gray', 'dodgerblue', 'black', 'burlywood4')")
            ...
            r("barcolors=c('red', 'orchid', 'palegreen', 'paleturquoise', 'peachpuff', 'royalblue', 'salmon', 'navy', 'magenta', 'dodgerblue', 'black')")
            r("barplot(as.matrix(data), horiz=F, beside=T, col=barcolors, main='Symmetric difference at %s OTUs', xlab='Difference', ylab='Percent', ylim=c(0, 100))" % file[:-4])
            r("mtext('%s')" % env[env_key])
            r("barnames=rownames")

            #r("barnames=c('Bayesian', 'PCoA (UU)', 'UPGMA (UU)', 'NJ (UU)', 'PCoA (WU)', 'UPGMA (WU)', 'NJ (WU)', 'PCoA (UUN)', 'UPGMA (UUN)', 'NJ (UUN)', 'PCoA (WUN)', 'UPGMA (WUN)', 'NJ (WUN)', 'PCoA (BC)', 'UPGMA (BC)', 'NJ (BC)')")
            r("barnames=c('Bayesian', 'PCoA (UU)', 'UPGMA (UU)', 'PCoA (WU)', 'UPGMA (WU)', 'PCoA (UUN)', 'UPGMA (UUN)', 'PCoA (WUN)', 'UPGMA (WUN)', 'PCoA (BC)', 'UPGMA (BC)')")

            r("legend('topright', inset=c(-0.35, 0), barnames, fill=barcolors, bty='n', cex=0.75)")
            r('dev.off()')
        except:
            traceback.print_exc()
